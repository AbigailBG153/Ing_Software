...
public class WeeklyScheduleService {
    ...
    public void deleteWeeklySchedule(Long id) {
        ...
        // Eliminar el horario semanal (esto también eliminará en cascada la lista de
        // horarios asociada)
        weeklyScheduleRepository.delete(weeklySchedule);
    }

    /// FUNCIONES ESPECIALES PARA ESPECIALISTA
    public WeeklyScheduleResponseDTO createWeeklySchedule(Long specialistProfileId,
            WeeklyScheduleRequestDTO requestDTO) {
        SpecialistProfile specialistProfile = specialistProfileRepository.findById(specialistProfileId)
                .orElseThrow(() -> new UserNotFound(
                        "Perfil de especialista no encontrado con el ID: " + specialistProfileId));
    ...
    public List<Schedule> generateSchedulesForDay(LocalDate date, DayOfWeek dayOfWeek, WeeklySchedule weeklySchedule) {
        ...
        while (startTime.isBefore(endTime)) {
            Schedule schedule = scheduleMapper
                    .convertToEntity(scheduleMapper.craeteScheduleRequestDTO(date, dayOfWeek, startTime));
    ...
    public WeeklyScheduleResponseDTO updateWeeklySchedule_Specialist(Long id, WeeklyScheduleRequestDTO requestDTO,
            Long specialistId) {
        SpecialistProfile specialistProfile = specialistProfileRepository.findById(specialistId)
                .orElseThrow(() -> new UserNotFound("Perfil de especialista no encontrado con el ID: " + specialistId));
        ...
        WeeklySchedule weeklySchedule = specialistProfile.getWeeklySchedules().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new UserNotFound("Weekly schedule not found with id: " + id)); // Asegurarse de
                                                                                                  // lanzar la excepción
                                                                                                  // correcta
