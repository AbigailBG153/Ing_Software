...
@Component({
  selector: 'app-list-top-specialists',
  templateUrl: './list-top-specialists.component.html',
  styleUrls: ['./list-top-specialists.component.css']
})
export class ListTopSpecialitsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.specialistService.getSpecialistProfilesByOccupation('NUTRICIONISTA').subscribe((data: SpecialistResponseDTO[]) => {
    ...
    this.specialistService.getSpecialistProfilesByOccupation('ENTRENADOR').subscribe((data: SpecialistResponseDTO[]) => {
      // Ordenar por score de mayor a menor
      data.sort((a, b) => (b.score || 0) - (a.score || 0));
      // Tomar los primeros 3
